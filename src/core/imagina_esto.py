...
Filosofía
---------
La detección se hace sobre los **SUBTÍTULOS YA GENERADOS**
(`materials.texts` + el `target_timerange` de su segmento), **NO** sobre el
texto de `escenas.txt`. Cada bloque de subtítulo ya tiene su rango temporal
(`start` + `duration`), así que ese rango ES exactamente la parte de la imagen
que hay que ocultar: no hay que emparejar escenas ↔ segmentos y no se puede
confundir una imagen con otra.

Flujo (4 fases, tal y como se ejecuta aquí):

| Fase | Qué hace | Dónde |
|---|---|---|
| **A** | el proyecto se genera normal (v1.3.0) y se escribe el JSON | `capcut_project.CapCutProject.generate` |
| **B** | leer el JSON, normalizar cada subtítulo y marcar los que tienen keyword | `aplicar` → `bloques()` + `_marcar()` |
| **C** | una keyword partida en 2-3 bloques contiguos marca **todos** los bloques | `_marcar()` |
| **D** | centrar el subtítulo, ocultar la imagen debajo y **verificar** | `centrar_segmento()`, `ocultar_rango()`, `_verificar()` |
...
curva y el zoom no se reinicie dentro de la imagen partida. Si la división
fallara, se recurre a la **opción de respaldo** (ocultar el segmento completo) y
se deja escrito por qué en el log.

FASE D.3 — verificación obligatoria
-----------------------------------
...
2. todos los segmentos de video que se solapan con su rango están ocultos
   (`visible == false` **y** `clip.alpha == 0.0`),
3. esos segmentos tienen un keyframe `KFTypeAlpha` con `values: [0.0]`.

Si algo falla se lanza `ImaginaEstoError` (el caller aborta la generación y no
deja el proyecto a medias).

Nota sobre `track_text_id`: el generador pasa el id de la pista de
**subtítulos** para que la pista del **watermark** (también `type == "text"`)
...
# FASE C: nunca se concatenan más de 3 bloques seguidos (evita falsos positivos
# al cruzar frases de escenas distintas).
MAX_BLOQUES_CONCATENADOS = 3

# Signos de puntuación que se ignoran al comparar (la lista de la especificación:
# ¿ ? ¡ ! . , ; : " ' … ). Se elimina todo lo que no sea letra ni dígito, lo que
# además cubre guiones y paréntesis.
_NO_ALFANUMERICO = re.compile(r"[^a-z0-9]+")


class ImaginaEstoError(RuntimeError):
    """Fallo de la verificación de la FASE D.3: se aborta la generación."""


# ---------------------------------------------------------------------------
# Normalización (idéntica para el texto del subtítulo y para las keywords)
# ---------------------------------------------------------------------------
def normalizar(texto: str | None) -> str:
    """minúsculas + sin tildes + sin puntuación + espacios colapsados.

    `"¿Supón  que…!"` → `"supon que"`. Se usa para los subtítulos y para
    `config.IMAGINA_ESTO_KEYWORDS`, así que da igual cómo se escriba la frase
    (con tildes, en mayúsculas o con signos)."""
    if not texto:
        return ""
...
# Las keywords se normalizan al importar (una sola vez) y se ordenan de más
# larga a más corta (y alfabéticamente entre iguales, para que el orden sea
# estable) para que, si dos coinciden a la vez, se informe de la más larga.
...
# ---------------------------------------------------------------------------
# FASE B — bloques de subtítulo
# ---------------------------------------------------------------------------
@dataclass
class Bloque:
    """Un bloque de subtítulo: su texto (ya normalizado) y su rango temporal."""
...
def bloques(content: dict, track_text_id: str | None = None) -> list[Bloque]:
    """FASE B: bloques de subtítulo con texto, tiempos y texto normalizado.
...
def _marcar(bloques_: list[Bloque]) -> None:
    ...
    2) si no aparece, se concatenan 2 y 3 bloques **contiguos** (rangos
       temporales seguidos, sin huecos) y se busca en la concatenación: si
    ...
    for largo in range(2, MAX_BLOQUES_CONCATENADOS + 1):
        for i in range(len(bloques_)):
            ...
            # Solo buscamos si hay alguna posibilidad de que la keyword esté partida.
            ...
            # CRÍTICO: El keyword no debe estar ya contenido en ningún subgrupo
            # contiguo más pequeño. Si lo está, ese subgrupo más pequeño es el
            # que debe marcarse (o ya se marcó), no este grupo más grande.
...
def centrar_segmento(seg: dict) -> None:
    ...
    `subtitles.SUBTITLE_Y` en los subtítulos normales."""
...
def _alpha_keyframe(value: float = 0.0, t_us: int = 0) -> dict:
    """Keyframe `KFTypeAlpha` con un único valor constante (3ª capa del ocultamiento)."""
    return {
        "id": _new_id(),
        "material_id": "",
...
def _slice_keyframes(kfs: list[dict], sub_start: int, sub_end: int) -> list[dict]:
    ...
    Los `time_offset` de un segmento son RELATIVOS a su propio inicio, así que al
    partir una imagen cada trozo debe seguir la **misma** curva de zoom / color
    grading / paneo (si no, el zoom se reiniciaría dentro de cada trozo). Se
    conservan los keyframes interiores y se añaden los dos extremos con el valor
    interpolado; los de un solo punto (los estáticos de color) siguen siendo un
    único keyframe constante."""
    out: list[dict] = []
...
def _ocultar_segmento(seg: dict) -> None:
    ...
    Se aplican JUNTAS porque la verificación visual de v1.4.0 mostró que el
...
def dividir_segmento(seg: dict, ini: int, fin: int) -> list[dict]:
    """Parte `seg` en [antes] [durante(oculto)] [después] dentro de [ini, fin).
...
def ocultar_rango(content: dict, ini: int, fin: int) -> int:
    ...
    Devuelve el nº de segmentos ocultos (los recién creados + los que ya estaban
    ocultos por un bloque anterior). Opción de respaldo: si la división de un
        ...
        for seg in (pista.get("segments") or []):
            ...
            try:
                ...
                log.warning(
                    ...
                    "se oculta COMPLETO (opción de respaldo).",
                    seg.get("id"), exc,
                )
...
# ---------------------------------------------------------------------------
# FASE D.3 — verificación obligatoria (vuelve a leer el JSON desde disco)
# ---------------------------------------------------------------------------
def _verificar(path: Path, esperados: list[Bloque],
    ...
    Devuelve la lista de errores (vacía si todo está bien). Cada error empieza
    ...
    for b in esperados:
        ...
        if b.afectado:
            if not es_centrado:
                errores.append(
                    f"bloque {b.idx} ({b.texto_norm!r}): con keyword pero NO está "
                ...
                for seg in solapados:
                    if not esta_oculto(seg):
                        errores.append(
                            ...
                            f"{seg.get('id')} [{rango(seg)[0]} .. {rango(seg)[1]}) NO está "
            ...
            if es_centrado:
                errores.append(
                    f"bloque {b.idx} ({b.texto_norm!r}): SIN keyword pero ESTÁ "
                    f"centrado (transform={transform})")
            for seg in solapados:
                if esta_oculto(seg):
                    errores.append(
                        ...
                        f"{rango(seg)[1]}) ESTÁ oculto")
    return errores


# ---------------------------------------------------------------------------
# Orquestación de las FASES B + C + D
# ---------------------------------------------------------------------------
...
def aplicar(
    ...
    Centra el texto de los bloques de subtítulo con keyword y oculta la imagen
    que hay debajo. Escribe el JSON en la raíz y en `Timelines/<id>/` (CapCut lee
    ambos) y **verifica el resultado leyéndolo de nuevo desde disco**. Devuelve
    el `content` resultante, o `None` si el proyecto no tiene subtítulos."""
    ...
    if not bloques_:
        log.info("[imagina-esto] sin bloques de subtítulo; se omite el post-proceso.")
        return None
    _marcar(bloques_)
    ...
    # D.3: se verifica SIEMPRE sobre lo escrito, leyéndolo de nuevo de disco.
    errores: list[str] = []
    for path in destinos:
    ...
    if errores:
        raise ImaginaEstoError(
            f"'imagina esto': la verificación del draft falló "
